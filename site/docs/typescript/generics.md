...
Узагальнене програмування в TypeScript — створення гнучких, типобезпечних компонентів.

## Базові Generics

```typescript
// Функція без generics — працює тільки з одним типом
function getFirst(items: number[]): number {
    return items[0];
}

// Generic функція — працює з будь-яким типом
...
const firstAuto = getFirst([true, false]);             // boolean (виведення типу)
```
...
// findById([1, 2, 3], 1); // Помилка! number не має поля id
```
...
// getProperty(user, "phone"); // Помилка! "phone" не існує
```
...
// Без вказання типу — використовується default
...
## Практичний приклад: Test Fixtures

```typescript
// Generic фабрика тестових даних
class TestFixture<T> {
    private defaults: T;
...
::: tip Generics для чистого коду
Generics допомагають уникнути дублювання коду зберігаючи типобезпечність. Використовуйте їх коли логіка однакова для різних типів.
:::
