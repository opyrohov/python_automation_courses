...
Типізація функцій у TypeScript — параметри, повернення значень, перевантаження та стрілочні функції.
...
function greet(name: string) {
    return `Hello, ${name}!`; // TypeScript розуміє: string
}
```
...
function launchBrowser(
    ...
    console.log(`Запуск: ${browser}, headless=${headless}`);
}
...
## Перевантаження (Overloads)

```typescript
// Різна поведінка для різних типів
...
```typescript
// Функція працює з будь-яким типом
...
async function withRetry(action: AsyncAction, retries: number = 3): Promise<void> {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            console.log(`Спроба ${i + 1} невдала, повтор...`);
        }
    }
}
```

::: tip Типізація для тестів
```typescript
// Типізована тестова функція
